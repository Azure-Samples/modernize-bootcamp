...
namespace eShopLite.AdminFx.Forms
{
    public partial class MainForm : Form
    {
        ...
        {
            base.OnLoad(e);
            Text = string.Format("eShopLite AdminFx - {0} - {1}", _settings.ProductsApiBaseUrl, LegacyAuthService.DescribeCurrentUser());
            ApplyPermissions();
            await LoadProductsAsync();
            LoadOrders();
        }

        // Client-side only: the Products API never sees who the user is.
        private void ApplyPermissions()
        {
            var canEdit = LegacyAuthService.IsEditor();
            ...
            if (!canEdit)
            {
                SetStatus("Signed in as a reader. Editing is disabled.");
            }
        }
