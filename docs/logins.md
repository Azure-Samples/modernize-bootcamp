...
Either account works for the sign-in and cart checks the labs ask you to run. These rows already exist in the shared `eShop` database; the app never creates them, because [Global.asax.cs](../src/app-modernization/caldova-retail-web-app/src/eShopLite.StoreFx/Global.asax.cs) disables the Entity Framework initializer so EF can never alter the database.
